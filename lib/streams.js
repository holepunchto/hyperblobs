...
class BlobWriteStream extends Writable {
  constructor(core, lock, opts = {}) {
    ...
    this.core = core
    this._addBlockMap = !!opts.blockMap
  ...
  async _append() {
    ...
    if (this._blockMap) {
      for (let i = 0; i < batch.length; i++) {
        this._blockMap.blocks.push({
          index: this.core.length + i,
          byteLength: batch[i].byteLength
        })
      }
    }
...
class BlockMapReadStream extends Readable {
  constructor(core, id, opts = {}) {
    ...
    const end = opts.end === undefined
      ? opts.length === undefined ? -1 : (start + opts.length)
      : opts.end + 1
  ...
  async _openp() {
    ...
    const [
      ...
      endIndex,
      endLength
    ] = seekBlockMap(this._blockMap, this._rangeStart, this._rangeEnd)
