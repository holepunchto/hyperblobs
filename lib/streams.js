...
class BlockMapReadStream extends Readable {
  ...
  _prefetch(index) {
    ...
    this._range = this.core.download({ blocks, linear: true })
  }
  ...
  async _readp() {
    ...
    if (!this._range && !this._noPrefetch) {
      ...
      if (!block && !this._range) this._prefetch(index)
    }
...
class BlobReadStream extends Readable {
  ...
  async _setEnd() {
    if (this._end === this.id.byteOffset + this.id.byteLength) {
      return this.id.blockOffset + this.id.blockLength
    }

    const result = await this.core.seek(this._end)
